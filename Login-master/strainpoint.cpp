...
strainpoint::strainpoint(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::strainpoint)
{
    ui->setupUi(this);

    ui->pointlineEdit->setPlaceholderText("输入要显示曲线的测点编号");


    QFile fweiyi("/Users/xjh/Desktop/Qt/Bradge_xjh/txt/strainpoint.txt");
...
strainpoint::~strainpoint()
{
    delete ui;
}

void strainpoint::on_returnpushButton_clicked()
{
    this->close();
    Exam *a =new Exam;
    a->show();
...
void strainpoint::on_delepushButton_clicked()
{
    //目前仅支持删除选中的当前行
        //获取当前条目
...
void strainpoint::on_savepushButton_clicked()
{
    ...
    //获取保存的文件名
        QString strFileSave = QFileDialog::getSaveFileName(
                    this,
                    tr("保存文件"),
                    tr("/Users/xjh/Desktop/strainPoint.txt"),
                    tr("Text Files(*.txt)")
                    );
...
void strainpoint::on_addpushButton_clicked()
{
    QString str[3];
...
void strainpoint::on_setsensorpushButton_clicked()
{
    this->close();
    addstrainsensor *a=new addstrainsensor;
    a->show();
    emit close();

}

void strainpoint::on_pushButton_clicked()
{
    QFile fweiyi("/Users/xjh/Desktop/Qt/Bradge_xjh/txt/strainpoint.txt");
    if(!fweiyi.open(QIODevice::ReadOnly | QIODevice::Text))
    {
    ...
    globaldata = ui->pointlineEdit->text();
